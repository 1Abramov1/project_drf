# Django REST Framework: Управление курсами и уроками

API для образовательной платформы с системой курсов, уроков, платежей и управления правами доступа.

## 🚀 Быстрый старт

```bash
# Клонирование и установка
git clone <repo-url>
cd project_drf
python -m venv venv
venv\Scripts\activate      # Windows
source venv/bin/activate   # Mac/Linux

# Установка зависимостей
pip install -r requirements.txt

# Настройка переменных окружения
cp .env.example .env
# Отредактируйте .env файл, добавьте Stripe ключи

# Настройка базы данных
python manage.py migrate
python manage.py createsuperuser
python manage.py runserver

📁 Структура проекта

· materials/ — Курсы и уроки (API, тесты, пагинация, Stripe оплата)
· users/ — Пользователи, платежи, аутентификация
· myproject/ — Настройки Django
· api/ — Дополнительные API endpoints

✅ Ключевые функции

· 🔐 JWT-аутентификация с обновлением токенов
· 👥 Кастомная модель пользователя (email вместо username)
· 🛡 Система прав доступа (модераторы, пользователи, администраторы)
· 💳 Stripe интеграция для онлайн-платежей
· 💰 Управление платежами с фильтрацией и сортировкой
· 📄 Пагинация для списков курсов и уроков
· ✅ Тестовое покрытие (запуск: coverage run manage.py test materials.tests)

💳 Stripe Интеграция

Проект включает полную интеграцию с Stripe для обработки платежей:

Настройка Stripe

1. Создайте аккаунт на stripe.com
2. Получите тестовые ключи из Stripe Dashboard
3. Добавьте в .env файл:
STRIPE_SECRET_KEY=sk_test_ваш_секретный_ключ
STRIPE_PUBLISHABLE_KEY=pk_test_ваш_публичный_ключ

Использование
# Пример создания платежной сессии через API
import requests

# 1. Получить JWT токен
token_resp = requests.post("http://localhost:8000/api/users/token/", 
                          json={"email": "user@example.com", "password": "password"})
token = token_resp.json()["access"]

# 2. Создать Stripe Checkout сессию
headers = {"Authorization": f"Bearer {token}"}
checkout_resp = requests.post(
    "http://localhost:8000/api/materials/courses/8/checkout/",
    headers=headers,
    json={}
)

# 3. Получить ссылку для оплаты
checkout_url = checkout_resp.json()["checkout_url"]
print(f"Ссылка для оплаты: {checkout_url}")

🔌 Основные эндпоинты

Метод Эндпоинт Описание Аутентификация
POST /api/users/token/ Получение JWT-токена ❌
POST /api/users/token/refresh/ Обновление JWT-токена ❌
GET /api/materials/courses/ Список курсов ✅
POST /api/materials/courses/{id}/checkout/ Создание Stripe сессии для оплаты курса ✅
POST /api/materials/lessons/ Создание урока ✅
GET /api/materials/courses/{id}/lessons/ Уроки курса ✅

🛠 Технологии

· Django 4.2+ & Django REST Framework
· JWT аутентификация (SimpleJWT)
· Stripe API для обработки платежей
· django-filter для фильтрации данных
· SQLite (разработка) / PostgreSQL (продакшен)
· drf-yasg для автоматической документации API

📦 Зависимости
Django==4.2.0
djangorestframework==3.14.0
djangorestframework-simplejwt==5.3.0
django-filter==23.3
stripe==8.0.0
python-dotenv==1.0.0
drf-yasg==1.21.7
django-extensions==3.2.3

🧪 Тестирование
# Запуск всех тестов
python manage.py test

# Запуск тестов с покрытием
coverage run manage.py test
coverage report
coverage html  # для HTML отчета

# Тестирование Stripe интеграции
python test_stripe_checkout.py

📋 Модели данных

Курс (Course)

· name - Название курса
· description - Описание курса
· price - Цена курса
· stripe_price_id - ID цены в Stripe
· owner - Владелец курса

Пользователь (CustomUser)

· email - Email (используется для входа)
· phone - Телефон
· city - Город
· avatar - Аватар

---

Проект в рамках обучения Django REST Framework

🤝 Вклад в проект

1. Форкните репозиторий
2. Создайте feature ветку (git checkout -b feature/AmazingFeature)
3. Закоммитьте изменения (git commit -m 'Add some AmazingFeature')
4. Запушьте ветку (git push origin feature/AmazingFeature)
5. Откройте Pull Request

📄 Лицензия

Этот проект распространяется под лицензией MIT.

👨‍💻 Автор

Александр Абрамов

🙏 Благодарности

· Команда Django за отличный фреймворк
· Сообщество Django REST Framework
· Stripe за удобный платежный API
· Все контрибьюторы проекта

⭐️ Не забудьте поставить звезду, если проект был полезеy!
